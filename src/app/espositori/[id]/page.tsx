        ...
        <div className="grid grid-2" style={{ marginTop: 24 }}>
          <section className="card">
            ...
            <form action={updateExhibitor} style={{ display: 'grid', gap: 14, marginTop: 24 }}>
              ...
              <label>Stato<select name="status" defaultValue={item.status} style={inputStyle}>{EXHIBITOR_STATUSES.map((status) => <option key={status.value} value={status.value}>{status.label}</option>)}</select></label>
              ...
              <div className="grid grid-2"><label>Referente<input name="contact_name" defaultValue={item.contact_name ?? ''} style={inputStyle} /></label><label>Email<input name="email" defaultValue={item.email ?? ''} style={inputStyle} /></label></div>
              <div className="grid grid-2"><label>Telefono<input name="phone" defaultValue={item.phone ?? ''} style={inputStyle} /></label><label>Provincia<input name="province" defaultValue={item.province ?? ''} style={inputStyle} /></label></div>
              <div className="grid grid-2"><label>Comune<input name="city" defaultValue={item.city ?? ''} style={inputStyle} /></label><label>Regione<input name="region" defaultValue={item.region ?? ''} style={inputStyle} /></label></div>
          ...
          <section className="card">
            ...
            <PaymentHistory payments={paymentRows} exhibitorId={item.id} expectedAmount={expectedAmount} />
          </section>
        </div>
      </div>
    </main>
...
const inputStyle: React.CSSProperties = { display: 'block', width: '100%', marginTop: 8, padding: 12, borderRadius: 12, border: '1px solid var(--border)', background: 'white' };
