...
export function GoogleSheetSyncPreview() {
  ...
  async function analyze() { setLoading(true); setMessage('Lettura Google Sheet in corso...'); const response = await fetch('/api/sync/google-sheet/preview'); const data = await safeJson(response); setLoading(false); if (!response.ok) setMessage(data.message || 'Errore analisi Google Sheet'); else { setItems(data.items ?? []); setMessage(`Analisi completata: ${(data.items ?? []).length} righe lette.`); } }
