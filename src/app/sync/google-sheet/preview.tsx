...
export function GoogleSheetSyncPreview() {
  ...
  async function analyze() {
    ...
    if (!response.ok) {
      ...
      setAnalysisState({ status: 'success', message: `Analisi completata: ${nextItems.length} righe lette.`, details: `Nuovi: ${creates} | Aggiornabili: ${updates} | Conflitti: ${conflicts}` });
    }
  }
  ...
  async function apply(overwriteConflicts: boolean) {
    ...
    setApplying(false);
    if (response.ok) {
      setImportState({ status: 'success', message: data.message || 'Import completato.', details: `Creati: ${data.created ?? 0} | Aggiornati: ${data.updated ?? 0} | Conflitti saltati: ${data.skippedConflicts ?? 0}` });
      await analyze();
    } else {
