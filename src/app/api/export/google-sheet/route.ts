...
export async function POST() {
  try {
    ...
    await ensureSheet(sheets, spreadsheetId, 'Espositori');
    await ensureSheet(sheets, spreadsheetId, 'Pagamenti');
    ...
    return NextResponse.json({ message: 'Google Sheet aggiornato correttamente', exhibitors: exhibitors?.length ?? 0, payments: payments?.length ?? 0 });
  } catch (error) {
