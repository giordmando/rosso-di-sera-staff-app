...
export async function POST() {
  ...
  const { data: exhibitors, error: exhibitorsError } = await supabase
    .from('exhibitors')
    .select('id, brand_name, company_name, contact_name, email, phone, city, province, region, status, products, internal_notes, created_at, updated_at')
    .order('created_at', { ascending: false });
  if (exhibitorsError) return NextResponse.json({ message: exhibitorsError.message }, { status: 400 });

  const { data: payments, error: paymentsError } = await supabase
    .from('payments')
    .select('id, paid_amount, expected_amount, payment_method, payment_date, receipt_received, notes, created_at, exhibitors(brand_name, email)')
    .order('created_at', { ascending: false });
  if (paymentsError) return NextResponse.json({ message: paymentsError.message }, { status: 400 });

  const exhibitorValues = [
    ['ID', 'Cantina', 'Ragione sociale', 'Referente', 'Email', 'Telefono', 'Comune', 'Provincia', 'Regione', 'Stato', 'Prodotti', 'Note interne', 'Creato il', 'Aggiornato il'],
    ...(exhibitors ?? []).map((item) => [item.id, item.brand_name, item.company_name, item.contact_name, item.email, item.phone, item.city, item.province, item.region, item.status, item.products, item.internal_notes, item.created_at, item.updated_at].map(value)),
  ];

  const paymentValues = [
    ['ID', 'Cantina', 'Email', 'Importo pagato', 'Quota prevista', 'Metodo', 'Data pagamento', 'Ricevuta', 'Note', 'Creato il'],
    ...(payments ?? []).map((item: any) => [item.id, item.exhibitors?.brand_name, item.exhibitors?.email, item.paid_amount, item.expected_amount, item.payment_method, item.payment_date, item.receipt_received ? 'Si' : 'No', item.notes, item.created_at].map(value)),
  ];
