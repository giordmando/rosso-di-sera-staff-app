...
export async function POST(request: Request) {
  ...
  await supabase.from('audit_logs').insert({ actor_email: body.email || null, action: 'candidature.create', entity_type: 'exhibitor', entity_id: data?.id ?? null, message: `Nuova candidatura ${body.brand_name || ''}`, metadata: { brandName: body.brand_name, province: payload.province, region: payload.region } });
