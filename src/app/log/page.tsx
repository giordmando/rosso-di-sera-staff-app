...
type PageSearchParams = Promise<{ q?: string; action?: string; page?: string }>;
...
function safeSearch(value: string) { return value.replaceAll('%', '').replaceAll(',', ' ').trim(); }

export default async function LogPage({ searchParams }: { searchParams: PageSearchParams }) {
  await requireAdmin();
  const params = await searchParams;
  const supabase = createSupabaseAdmin();
  const q = safeSearch(String(params.q ?? ''));
